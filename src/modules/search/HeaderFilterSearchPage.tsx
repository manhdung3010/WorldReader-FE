...
import TabFilters from "../book/TabFilters";
import { getCategoryProduct } from "@/services/category-product.service";
import { useQuery } from "@tanstack/react-query";

interface HeaderFilterSearchPageProps {
  className?: string;
...
const HeaderFilterSearchPage: FC<HeaderFilterSearchPageProps> = ({
  ...
  const {
    ...
    ["CATEGORY_PRODUCT"],
    async () => {
      const response: any = await getCategoryProduct({});
  ...
  const handleCategorySelect = (categoryId: string) => {
    ...
    if (handleFilterChange) {
      handleFilterChange({
        categories: categoryId === "all" ? null : [categoryId],
      });
    }
  };
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between space-y-6 lg:space-y-0 lg:space-x-2 ">
        {isLoading ? (
          ...
          >
            ...
            {categoryResponse?.data?.map((item: any, index: number) => (
              <NavItem
                key={index}
